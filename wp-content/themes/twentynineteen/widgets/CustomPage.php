<?php
class portfolio__Widget__Custom_Page extends WP_Widget
{
	public function __construct()
	{
		parent::__construct('portfolio__Widget__Custom_Page',
			__( '(Portfolio) [PAGE] Custom', 'portfolio' ),
			array( 'description' => __( 'About, resume, contact etc.', 'portfolio' ) ) );
	}
	...
	public function widget( $args, $instance )
	{
		...
		if ( $loop_custom_page->have_posts() ) :
			while ( $loop_custom_page->have_posts() ) : $loop_custom_page->the_post();
				...
				?>
				<section id="<?php echo esc_attr( $custom_page_slug ); ?>" class="pt-page page-layout <?php if ( has_post_thumbnail() ) { echo 'has-bg-img' . '"' . ' style="background-image: url( ' . wp_get_attachment_url( $attachment_id ) . ' );"'; } else { echo '"'; } ?>>
								<div class="content">
...
function portfolio__Widget__About_fuc()
{
	return register_widget("portfolio__Widget__Custom_Page");
}

add_action('widgets_init', 'portfolio__Widget__About_fuc');
?>